...
function updateMainStats(data) {
    ...
    let latestWithTemp = data.find(d => d.temperature !== '-' && d.temperature !== '');
    let latest = data[0];
    
    // Update temperature
    if (latestWithTemp) {
        document.getElementById('temperature').innerHTML = 
            `${latestWithTemp.temperature} <span class="unit">°C</span>`;
        document.getElementById('temp-time').textContent = 
            `Měřeno: ${latestWithTemp.dateTime}`;
    }
    
    // Update water level
    if (latest.level !== '-') {
        document.getElementById('water-level').innerHTML = 
            `${latest.level} <span class="unit">cm</span>`;
        document.getElementById('level-time').textContent = 
            `Měřeno: ${latest.dateTime}`;
    }
    
    // Update flow rate
    if (latest.flow !== '-') {
        document.getElementById('flow-rate').innerHTML = 
            `${latest.flow} <span class="unit">m³/s</span>`;
        document.getElementById('flow-time').textContent = 
            `Měřeno: ${latest.dateTime}`;
    }
    
    // Update last update time
    document.getElementById('lastUpdate').textContent = new Date().toLocaleString('cs-CZ');
}
...
function updateDataTable(data) {
    ...
    if (!data || data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="loading-cell">Žádná data k dispozici</td></tr>';
        return;
    }
...
function updateTemperatureChart(data) {
    ...
    const tempData = data.filter(d => d.temperature !== '-' && d.temperature !== '').slice(0, 48).reverse();
    
    const labels = tempData.map(d => {
        const parts = d.dateTime.split(' ');
        return parts.length >= 2 ? parts[0].substring(0, 5) + ' ' + parts[1] : d.dateTime;
    });
...
function updateWaterLevelChart(data) {
    ...
    const levelData = data.filter(d => d.level !== '-').slice(0, 48).reverse();
...
function updateFlowChart(data) {
    ...
    // Get last 48 data points
    const flowData = data.filter(d => d.flow !== '-').slice(0, 48).reverse();
...
function createModalChart(type) {
    ...
    if (type === 'temperature') {
        filteredData = currentData.filter(d => d.temperature !== '-' && d.temperature !== '').slice(0, 24).reverse();
        ...
        filteredData = currentData.filter(d => d.level !== '-').slice(0, 24).reverse();
        labels = filteredData.map(d => {
            const parts = d.dateTime.split(' ');
            return parts.length >= 2 ? parts[0].substring(0, 5) + ' ' + parts[1] : d.dateTime;
        });
        ...
        filteredData = currentData.filter(d => d.flow !== '-').slice(0, 24).reverse();
        labels = filteredData.map(d => {
            const parts = d.dateTime.split(' ');
            return parts.length >= 2 ? parts[0].substring(0, 5) + ' ' + parts[1] : d.dateTime;
        });
